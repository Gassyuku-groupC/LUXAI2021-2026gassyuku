...
class SurvivalResearchReward(FullGameRewardSpace):
    ...
    def compute_rewards(self, game_state: Game, done: bool) -> Tuple[float, float]:
        ...
        buffer_deficit = np.maximum(self.min_buffer_nights - new_fuel_buffer, 0.)
        ...
        reward_items_dict = {
            ...
            "unit": new_unit_count - self.unit_count,
            "research": np.maximum(research_diff, 0.),
            ...
            "buffer_gain": np.maximum(new_fuel_buffer - self.fuel_buffer, 0.),
            "buffer_deficit": -buffer_deficit * self.should_check_fuel_buffer(game_state),
            "unsafe_expansion": -np.maximum(city_diff, 0.) * (buffer_deficit > 0.).astype(float),
    ...
    def count_fuel_buffer(self, game_state: Game) -> np.ndarray:
        return np.array([
            min([city.fuel / max(city.light_upkeep, 1.) for city in player.cities.values()] + [0.])
            for player in game_state.players
        ])
