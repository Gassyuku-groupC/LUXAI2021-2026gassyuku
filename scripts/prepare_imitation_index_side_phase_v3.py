#!/usr/bin/env python3
"""Create a side/phase failure-aware imitation index."""
...
def apply_side_phase_feedback(
    ...
    row["side_phase_severity"] = "0"
    row["side_phase_scale"] = "1.0000"
    if not modifier:
        return row

    severity = int(modifier.get("severity", 0))
    ...
    is_safe_example = (
        future_loss == 0
        and night_loss_next == 0
        and min_fuel is not None
        and min_fuel >= args.safe_min_fuel_turns
    )
    is_risky_example = (
        future_loss > 0
    ...
    elif severity >= 2 and min_fuel is not None and min_fuel >= args.safe_min_fuel_turns:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Create a side/phase failure-aware imitation index.")
    parser.add_argument("--input", type=Path, default=Path("dataset/processed/imitation_index_hq.csv"))
    parser.add_argument("--output", type=Path, default=Path("dataset/processed/imitation_index_side_phase_v3.csv"))
    parser.add_argument("--modifiers", type=Path, default=Path("outputs/risk_feature_logs/side_phase_failure_modifiers.json"))
    ...
    parser.add_argument("--safe-min-fuel-turns", type=float, default=10.0)
    parser.add_argument("--risky-min-fuel-turns", type=float, default=5.0)
    parser.add_argument("--failure-safe-scale-moderate", type=float, default=1.15)
    parser.add_argument("--failure-safe-scale-high", type=float, default=1.35)
    parser.add_argument("--failure-safe-scale-severe", type=float, default=1.60)
    parser.add_argument("--failure-risk-scale-moderate", type=float, default=0.85)
    parser.add_argument("--failure-risk-scale-high", type=float, default=0.65)
    parser.add_argument("--failure-risk-scale-severe", type=float, default=0.45)
    parser.add_argument("--safe-pre-night-extra-scale", type=float, default=1.15)
    parser.add_argument("--buffered-same-phase-scale", type=float, default=1.10)
    parser.add_argument("--min-weight", type=float, default=0.10)
    parser.add_argument("--max-weight", type=float, default=4.5)

    # Base risk-aware parameters, kept aligned with v2 defaults unless overridden here.
    ...
    parser.add_argument("--safe-pre-night-buffer-scale", type=float, default=1.35)
    parser.add_argument("--pre-night-bw-severe-extra-scale", type=float, default=0.65)
    ...
    for replay_path, replay_rows in grouped.items():
        ...
        for base_row in replay_rows:
            ...
            final_row = apply_side_phase_feedback(risk_row, replay, state_cache, modifiers, args)
